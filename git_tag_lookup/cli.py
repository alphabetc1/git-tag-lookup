...
from .core import GitTagLookupError, find_earliest_tag_for_commit, search_commits_by_keyword
...
Examples:
  ...
  git-tag-lookup --repo /path/to/repo --key "fix bug" -n 10
    ...
    parser.add_argument("--repo", required=True, help="Git repository URL or local directory path")

    # Mutually exclusive group for commit and keyword
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--commit", help="Commit hash to find the earliest tag containing it")
    group.add_argument("--key", dest="keyword", help="Keyword to search in commit messages")
